...
public interface InstructionBEScreen extends InstructionScreen {
  static void instructionBlockEntity(
    InstructionBEScreen screen,
    ...
    if (
      ...
      blockEntity instanceof IInstructionBlockEntity
    ) {
      NetworkManager.sendToServer(
        OEPackets.BLOCK_ENTITY_INSTRUCTION,
        new OEPackets.BlockEntityInstructionMessage(
          screen.getInstructionID(),
          BlockEntityExistence.getByBlockEntity(blockEntity),
          name,
          data
        ).toRFBB()
      );
